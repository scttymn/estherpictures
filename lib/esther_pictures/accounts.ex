defmodule EstherPictures.Accounts do
  @moduledoc """
  ...
  alias EstherPictures.Accounts.{User, UserToken, UserNotifier}
  ...
  @doc "Lists all users, admins first, then by email."
  def list_users do
    from(u in User, order_by: [asc: u.role, asc: u.email])
    |> Repo.all()
  end

  @doc "Counts the total number of user accounts."
  def count_users, do: Repo.aggregate(User, :count, :id)

  @doc "Returns true when at least one account exists (used to gate bootstrap registration)."
  def any_users?, do: count_users() > 0

  @doc "Returns true when the user has the admin role."
  ...
  @doc """
  Registers the very first account as the site administrator.

  This is the one-time bootstrap path: it only succeeds when no accounts exist
  yet. The account is created with the admin role and an email + password.
  """
  def register_first_admin(attrs) do
    if any_users?() do
      ...
      |> User.registration_changeset(
        Map.merge(normalize_attrs(attrs), %{"role" => "admin", "must_change_password" => false})
      )
      |> Repo.insert()
    end
  end
  ...
  @doc """
  Returns a changeset for the bootstrap admin registration form.
  """
  def change_first_admin_registration(user \\ %User{}, attrs \\ %{}) do
  ...
  def create_editor(attrs) do
    ...
    role = if attrs["role"] in User.roles(), do: attrs["role"], else: "editor"
  ...
  @doc "Returns a changeset for the invite-editor form."
  def change_editor(user \\ %User{}, attrs \\ %{}) do
    User.registration_changeset(user, attrs, hash_password: false, validate_unique: false)
  end

  @doc "Deletes a user account."
  ...
  See `EstherPictures.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `EstherPictures.Accounts.User.password_changeset/3` for a list of supported options.
